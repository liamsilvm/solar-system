...
class Body{ 
  ...
  draw(x_offset, y_offset){ 
    fill(this.color)
    ellipse(this.x + x_offset, this.y + y_offset, this.radius)
  }
...
const scalingFactor = 1.3; // Adjust this value as needed
...
let mars = new Body('mars', mars_mass, 2, Math.PI/2, 3.4 * scalingFactor, 40, 40, [255, 0, 0]);
let jupiter = new Body('jupiter', jupiter_mass, 2, Math.PI/2, 15 * scalingFactor, 30, 30, [150, 100, 50]);
let saturn = new Body('saturn', saturn_mass, 2, Math.PI/2, 14 * scalingFactor, -120, 120, [210, 180, 140]);
...
function renderAstroids(count, spacing, init_coords){ 
  let x_increment = 0
...
//mass, speed, direction, radius, x, y, color
  for(let i = 0; i < count; i++){ 
    ...
    objects.push(new Body('astroid',.5, 200, Math.PI/2, 2, init_coords[0], init_coords[1] + x_increment, [255,255,255]))
  }
}
